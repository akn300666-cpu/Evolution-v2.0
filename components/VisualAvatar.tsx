...
const VisualAvatar: React.FC<VisualAvatarProps> = ({ isThinking, emotion }) => {
  ...
  return (
    <div className="relative w-24 h-24 flex items-center justify-center">
      ...
      <div className={`relative w-20 h-20 rounded-full border-2 overflow-hidden shadow-2xl flex items-center justify-center transition-all duration-300 ${getBorderColor()}`}>
      ...
      {/* Status Dot */}
      <div className={`absolute bottom-0 right-1 w-3 h-3 rounded-full border-2 border-slate-900 transition-colors duration-300 ${isThinking ? 'bg-fuchsia-400 animate-ping' : 'hidden'}`}></div>
      <div className={`absolute bottom-0 right-1 w-3 h-3 rounded-full border-2 border-slate-900 transition-colors duration-300 ${getStatusColor()}`}></div>
    </div>
  );
};
